...
predict(m1,iris) #决策树，要计算的数据集

table(iris$Species, predict(m1),dnn=c("实际值","计算值"))
prop.table(table(iris$Species, predict(m1),dnn=c("实际值","计算值")))
prop.table(table(iris$Species, predict(m1),dnn=c("实际值","计算值")),1)
prop.table(table(iris$Species, predict(m1),dnn=c("实际值","计算值")),1)*100
prop.table(table(iris$Species, predict(m1),dnn=c("实际值","计算值")),2)
summary(m1)
